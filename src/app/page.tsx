...
export default function HomePage() {
  ...
  const mealsToShow = useMemo(() => {
    const allMeals = debouncedSearchTerm ? searchResults || [] : mealsData?.meals || [];
    return allMeals.filter((meal: Meal) =>
      selectedCategory ? meal.strCategory.toLowerCase() === selectedCategory.toLowerCase() : true
    );
  ...
  const categories = useMemo(() => {
    ...
    return Array.from(new Set(mealsData.meals.map((meal: Meal) => meal.strCategory)));
  }, [mealsData]);
        ...
        displayedMeals.map((meal: Meal) => (
          <li key={meal.idMeal} className={styles.card}>
            ...
            <div className={styles.btnWrap}>
              ...
              >
                {favorites.some((fav: Meal) => fav.idMeal === meal.idMeal) ? "Збережено" : "Зберегти"}
              </button>
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1)
          ...
          .map((page, index, arr) => (
            <div className={styles.pageItem} key={page}>
              {index > 0 && page !== arr[index - 1] + 1 && <span className={styles.dots}>...</span>}
              <button
